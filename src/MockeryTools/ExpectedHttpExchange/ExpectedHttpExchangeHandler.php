...
/**
 * @phpstan-import-type TRequestOptions from ExpectedHttpExchangeFactory
 */
class ExpectedHttpExchangeHandler
{
    /**
     * @var ExpectedHttpExchangeScenarioInterface[]
     */
    private array $expectedExchanges = [];

    private ?RequestDiffComputerInterface $requestDiffComputer;


    public function __construct(?RequestDiffComputerInterface $requestDiffComputer)
    {
        $this->requestDiffComputer = $requestDiffComputer;
    }
    ...
    private function handleRequest(RequestInterface $actualRequest): PromiseInterface
    {
        foreach ($this->expectedExchanges as $key => $expectedExchange) {
            ...
            if ($areRequestsSame) {
                ...
                if ($expectedExchange instanceof ExpectedFailedHttpExchange) {
                    return Create::rejectionFor($expectedExchange->getException());
                }

                assert($expectedExchange instanceof ExpectedHttpExchange);
        ...
        $this->requestDiffComputer?->outputRequestDiffs($actualRequest, $this->expectedExchanges);

        throw new NoMatchingExpectationException(
            sprintf(
    ...
    public function expectExchange(ExpectedHttpExchangeScenarioInterface $expectedExchange): void
    {
    ...
    /**
     * @return ExpectedHttpExchangeScenarioInterface[]
     */
    ...
    private function areRequestsHeadersSame(RequestInterface $expectedRequest, RequestInterface $actualRequest): bool
    {
         ...
         *
         * @see MultipartStream contrustor.
         */
        if ($actualRequest->getBody() instanceof MultipartStream) {
            unset($actualRequestHeaders['Content-Type'], $expectedRequestHeaders['Content-Type']);
        }
    ...
    private function areRequestsJsonBodiesSame(string $expectedRequestBody, string $actualRequestBody): bool
    {
        ...
        assert(is_array($expectedRequestBody));
        $actualRequestBody = Json::decode($actualRequestBody, Json::FORCE_ARRAY);
        assert(is_array($actualRequestBody));
    ...
    private function createRejectedPromise(ExpectedHttpExchange $expectedExchange): RejectedPromise
    {
